...
Following the pre-registration and methodological repair in Development V2, the **held-out evaluation cohort (`seeds 12000..12099`, $N = 100$)** was unsealed and executed across all 15 frozen environment variants and 6 controller architectures ($9,000$ total closed-loop episodes).

The held-out results **replicate the development findings with exact statistical fidelity**:

1. **Generalization Performance:**  
   - `DELTAX_EXECUTIVE` achieved a **73.3% generalization rate** (11 of 15 environments solved at $\ge 80\%$, with 100.0% goal discovery in all 11 solved environments).
...
2. **Causal Failure Attribution:**  
   Across all 400 failed episodes for DeltaX, **100.0% were caused by `SUBSTRATE_CANDIDATE_ABSENCE`**. In every failed environment (`ENV_1B_TRUE_RIGHT_REQUIRED`, `ENV_POLARITY_RIGHT`, `ENV_CHOICE_WITH_REVERSAL`, `ENV_TEMPORAL_NON_MARKOVIAN`), a right turn was physically mandatory, but descending neuron telemetry shows `turn_right` had $0.0\text{ Hz}$ raw activation and $0.000$ strength. There were **0 executive selection failures**.
3. **True Left/Right Polarity Dissociation:**  
   In the matched geometric reflection pair, DeltaX achieved **100.0% [96.3–100.0%] in `ENV_POLARITY_LEFT`** and **0.0% [0.0–3.7%] in `ENV_POLARITY_RIGHT`**.
...
5. **State Reset Invariance:**  
   `DELTAX_EXECUTIVE` (continuous retained state), `DELTAX_TRIAL_RESET` (inter-trial reset), and `DELTAX_STEP_RESET` (stateless per-tick session isolation) achieved **bit-exact identical performance** across all 9,000 episodes (11 / 15 environments, 73.33% mean goal rate). Retained executive state produced zero measurable advantage.
6. **Executive Decision Integrity:**  
   Across all $54,000$ executive decision ticks in the held-out evaluation, **Executive Selected-ID Match Rate was 100.00%** ($54,000 / 54,000$) and **Fallback Rate was 0.00%** ($0 / 54,000$).

---

## 2. Factorial Held-Out Results Matrix (Seeds 12000..12099, N=100)

Values reported as `Both Goals Reached % [Wilson 95% CI]`:
...
- **Local Trap Navigation (`ENV_2_MIRROR_HORIZONTAL`):** Simple Reflex degraded to **63.0%** goal discovery due to rigid reactive sequencing, whereas DeltaX maintained **100.0%**.
- **Left-Compatible Stability:** On all 11 left-compatible environments, DeltaX achieved **100.0% reliability with 0 collisions**, whereas Simple Reflex suffered 1–8% failures across environments.
...
- DeltaX provides the essential candidate arbitration required to translate raw connectome activations into coherent goal-directed locomotion.

### C & D. DELTAX_EXECUTIVE vs DELTAX_TRIAL_RESET vs DELTAX_STEP_RESET
- All three conditions produced **bit-exact identical trajectories, action counts, and goal discovery** across all 15 environments ($11 / 15$, 73.33%).
- **Causal Conclusion:** In this deterministic sensory-motor benchmark, cross-trial retained executive state provided **no measurable advantage** over trial-reset or step-reset stateless execution.
...
- **Causal Conclusion:** The connectome substrate possesses an endogenous leftward turning bias under lateralized obstacle contact.

### F. Left-Compatible vs Right-Required Worlds
- **Left-Compatible Environments (11):** DeltaX achieved **100.0%** (1,100 / 1,100 episodes).
- **Right-Required Environments (4):** DeltaX achieved **0.0%** (0 / 400 episodes).
...
- **Executive Selection Failures:** **0 across all 4,500 executive episodes**.
...
### Supported Claims:
1. Under `READOUT_C_INDEPENDENT_AXES` and lateralized sensory transduction, DeltaX executive navigation generalizes with **100.0% reliability** across a broad family of 11 left-compatible spatial transformations, obstacle shifts, hazard gradients, dead ends, and orientations.
2. DeltaX executive arbitration is strictly necessary to prevent connectome rotational limit cycles.
...
1. **General Bilateral Steering:** The unadapted connectome does not possess general right-turn motor competence under these geometries.
2. **Executive Memory Advantage:** Retained executive state across recurrence trials provided no advantage over stateless step-reset.
